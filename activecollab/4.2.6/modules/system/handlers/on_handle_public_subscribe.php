<?php
   ...
   */
  function system_handle_on_handle_public_subscribe($notification, $parts, &$unsubscribed, &$message, &$undo_url) {
      ...
      if ($user_id && $subscription_code) {
        ...
        if($user instanceof User && $user->getState() >= STATE_VISIBLE) {
          if (strtoupper($user->getAdditionalProperty('subscription_code')) == $subscription_code) {
            ConfigOptions::setValueFor('morning_paper_enabled', $user, true);

            $unsubscribed = true;
            $message = lang(':user_name has been successfully added to Morning Paper list', array(
              'user_name' => $user->getDisplayName(),
            ));

            $undo_url = Router::assemble('public_notifications_unsubscribe', array(
  ...
  } // system_handle_on_handle_public_subscribe