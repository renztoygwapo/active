...
{foreach from=$_groupped_new_objects item=_details}
  ...
  {if is_array($_details)}
    <td class="star">{favorite_object object=$_details.parent user=$logged_user}</td>
    <td class="name">
      {$_details.parent->getVerboseType()}: {object_link object=$_details.parent}
      <ul>
      {foreach from=$_details.objects item=_object}
        <li>{$_object->getVerboseType()}: {object_link object=$_object} <span class="details">{lang}by{/lang} {user_link user=$_object->getCreatedBy()}</span></li>
      {/foreach}
      </ul>
    </td>
  ...
  {else}
    <td class="star">{favorite_object object=$_details user=$logged_user}</td>
    <td class="name">{$_details->getVerboseType()}: {object_link object=$_details}</td>
