<div id="object_main_info" class="object_info">
  <h1>{lang}Milestone{/lang}: {$active_milestone->getName()}</h1>
</div>
    ...
    {if is_foreachable($objects)}
      <div class="object_info">
        ...
        <tr>
          ...
          <td class="column_name"><a href="../{$object->getModule()|strtolower}/{$object->getType()|strtolower}_{$object->getId()}.html">{$object->getName()}</a></td>
          <td class="column_date">{$object->getCreatedOn()|date}</td>
          <td class="column_author">{$object->getCreatedByName()}</td>
        </tr>
        {/foreach}
        </table>
      </div>
