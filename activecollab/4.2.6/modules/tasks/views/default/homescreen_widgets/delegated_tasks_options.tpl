<div id="homescreen_widget_{$widget->getId()}" class="task_filter_options_wrapper">
  ...
  {wrap field=task_filter_delegated_by class='task_filter_options_delegated_by_filter'}
    {select name='homescreen_widget[delegated_by_filter]' label='Delegated By' class="picker"}
      <option value="logged_user" {if $widget_data.delegated_by_filter == 'logged_user'}selected="selected"{/if}>{lang}Logged User{/lang}</option>
      <option value="selected" {if $widget_data.delegated_by_filter == 'selected'}selected="selected"{/if}>{lang}Selected User{/lang}</option>
    {/select} {select_user name='homescreen_widget[delegated_by_user_id]' exclude_ids=$widget_data.exclude_ids value=$widget_data.delegated_by_user_id user=$user}
  {/wrap}
...
<script type="text/javascript">
  $('#homescreen_widget_{$widget->getId()}').each(function() {
    ...
    // Delegated by filter
    if(wrapper.find('div.task_filter_options_delegated_by_filter select.picker').val() != 'selected') {
      wrapper.find('div.task_filter_options_delegated_by_filter select.select_user').hide();
    } // if

    wrapper.find('div.task_filter_options_delegated_by_filter select.picker').change(function() {
      if($(this).val() == 'selected') {
        wrapper.find('div.task_filter_options_delegated_by_filter select.select_user').show();
      } else {
        wrapper.find('div.task_filter_options_delegated_by_filter select.select_user').hide();
      } // if
    });
