<?php
  ...
  /**
   * Open vs completed tasks report
   *
   * @package activeCollab.modules.tasks
   * @subpackage models
   */
  class OpenVsCompletedTasksReport extends TasksAnalyzerReport {
      ...
      if($total_tasks) {
        ...
        DateValue::iterateWeekly($start_from, $go_to, function(DateTimeValue $week_start, DateTimeValue $week_end) use (&$weekly_data, $conditions, $project_objects_table) {
          ...
          $weekly_data[] = array(
            ...
            'total_tasks' => (integer) DB::executeFirstCell("SELECT COUNT(id) FROM $project_objects_table WHERE (created_on <= $escaped_end) AND ($conditions)"), // All tasks that were created before end of this week
            'completed_tasks' => (integer) DB::executeFirstCell("SELECT COUNT(id) FROM $project_objects_table WHERE (completed_on IS NOT NULL AND completed_on <= $escaped_end) AND ($conditions)"), // Total completed tasks by the end of this week
          );
    ...
    function runForExport(IUser $user, $additional = null) {
      ...
      if($data) {
        ...
        foreach($data as $v) {
          $this->exportWriteLine(array($v['year'], $v['week'], DateValue::makeFromTimestamp($v['week_start_timestamp'])->toMySQL(), $v['week_start_timestamp'], DateValue::makeFromTimestamp($v['week_end_timestamp'])->toMySQL(), $v['week_end_timestamp'], $v['total_tasks'], ($v['total_tasks'] - $v['completed_tasks']), $v['completed_tasks']));
        } // foreach
    ...
    function getRoutingContext() {
      return 'open_vs_completed_tasks_report';
    } // getRoutingContext
    ...
    function getRoutingContextParams() {
      return array('open_vs_completed_tasks_report_id' => $this->getId());
